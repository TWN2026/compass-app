...
def sql(query, params=None):
    ...
    try:
        ...
        print(f"ERR {query[:60]}... => {err.get('message')}")
        return None
...
b1_data = {
  "plan": {
    ...
    "threeYear": {"date": "2028-01-01", "revenue": "$20M", "profit": "$4M",
      "measurables": [{"id":1,"label":"Customers","value":"500+"},{"id":2,"label":"Team Size","value":"80+"},{"id":3,"label":"NPS","value":"72+"}],
      "possibilities": ["Expand to APAC","Launch mobile app","Strategic acquisition"]},
    "oneYear": {"date": "2026-01-01", "revenue": "$8M", "profit": "$1.2M",
      "measurables": [{"id":1,"label":"ARR","value":"$8M"},{"id":2,"label":"Headcount","value":"40"}],
      "goals": ["Hire VP of Sales","Launch v3 platform","Expand to 3 new verticals","Achieve SOC 2 compliance"]},
    "quarterly": {"date": "2025-06-30", "revenue": "$2M", "profit": "$300K",
      "measurables": [{"id":1,"label":"New MRR","value":"$150K"},{"id":2,"label":"Churn","value":"<2%"}],
      "goal": "Nail the enterprise pilot program with 5 anchor clients"},
    "longTermIssues": [
      {"id":1,"title":"Pricing model needs revision for enterprise tier","owner":"Sarah Mitchell"},
      {"id":2,"title":"Engineering hiring pipeline is too slow","owner":"James Chen"}
    ]
  },
  "orgChart": {"seats": [
    {"id":"s1","title":"Visionary / CEO",     "person":"Sarah Mitchell","accountabilities":["Company vision","Culture","Key relationships"],"parentId": None},
...
b2_data = {
  "plan": {
    "coreValues": [
      {"id":1,"title":"Transparency","desc":"Radical honesty, always."},
      {"id":2,"title":"Speed",       "desc":"Move fast, iterate faster."},
      {"id":3,"title":"Excellence",  "desc":"Never good enough until it's great."}
    ],
    ...
    "threeYear": {"date":"2028-01-01","revenue":"$12M","profit":"$2M",
      "measurables":[{"id":1,"label":"Customers","value":"1,000+"},{"id":2,"label":"Team","value":"50+"}],
      "possibilities":["Launch in EU","Acquire competitor","Go public"]},
    "oneYear": {"date":"2026-01-01","revenue":"$1.5M","profit":"$200K",
      "measurables":[{"id":1,"label":"ARR","value":"$1.5M"}],
      "goals":["Close Series A","Hire VP Engineering","Launch EU expansion"]},
    "quarterly": {"date":"2025-06-30","revenue":"$350K","profit":"$50K",
      "measurables":[{"id":1,"label":"New Logos","value":"20"}],
      "goal":"Close Series A and sign first enterprise deal"},
    "longTermIssues":[{"id":1,"title":"Product-market fit in EU still unclear","owner":"James Turner"}]
  },
  "orgChart": {"seats": [
    {"id":"s1","title":"CEO / Founder","person":"James Turner","accountabilities":["Vision","Fundraising","Strategy"],"parentId":None},
  ...
  "scorecard": [
    {"id":1,"metric":"MRR","owner":"James Turner","goal":"120000","goalType":"min","freq":"monthly","values":[{"wk":"Jan","v":98000},{"wk":"Feb","v":103000},{"wk":"Mar","v":108000},{"wk":"Apr","v":121000}]}
  ],
...
# Insert businesses
sql("INSERT INTO businesses (id, company, industry, color, data) VALUES ($1,$2,$3,$4,$5::jsonb) ON CONFLICT (id) DO NOTHING",
    ["b1", "Acme Corp", "B2B SaaS", "#7a9e7e", json.dumps(b1_data)])
sql("INSERT INTO businesses (id, company, industry, color, data) VALUES ($1,$2,$3,$4,$5::jsonb) ON CONFLICT (id) DO NOTHING",
    ["b2", "Vertex Solutions", "HR Tech", "#c9a84c", json.dumps(b2_data)])

# Insert users
users = [
  ("b1-u1","b1","Sarah Mitchell","sarah@acme.com","sarah.acme","acme123","owner"),
  ("b1-u2","b1","James Chen",    "james@acme.com","james.acme","acme456","member"),
  ("b1-u3","b1","Lisa Wong",     "lisa@acme.com", "lisa.acme", "acme789","member"),
  ("b2-u1","b2","James Turner",  "james@vertex.com","james.vertex","vertex123","owner"),
  ("b2-u2","b2","Amy Lee",       "amy@vertex.com",  "amy.vertex",  "vertex456","member"),
]
for u in users:
    sql("INSERT INTO users (id,business_id,name,email,username,password,role) VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (username) DO NOTHING", list(u))

...
const SEED_BUSINESSES = [
  ...
  },
  {
    ...
    data: {
      ...
      todos: [
        { id: 1, title: 'Prepare pitch deck v4', owner: 'James Turner', dueDate: '2025-03-15', done: false },
        { id: 2, title: 'Draft CS playbook',     owner: 'Amy Lee',      dueDate: '2025-03-20', done: false },
      ],
...
const COACH = { id: 'coach', name: 'Dean (Coach)', email: 'dean@coaching.com', username: 'coach', password: 'coach123', role: 'coach' };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Seed coach
    const coachHash = await bcrypt.hash(COACH.password, 10);
    await sql`
      INSERT INTO users (id, business_id, name, email, username, password, role)
      VALUES (${COACH.id}, NULL, ${COACH.name}, ${COACH.email}, ${COACH.username}, ${coachHash}, 'coach')
    ...
    for (const biz of SEED_BUSINESSES) {
      await sql`
        ...
        VALUES (${biz.id}, ${biz.company}, ${biz.industry}, ${biz.color}, ${JSON.stringify(biz.data)})
      ...
      for (const u of biz.users) {
        ...
        await sql`
          INSERT INTO users (id, business_id, name, email, username, password, role)
          VALUES (${u.id}, ${biz.id}, ${u.name}, ${u.email}, ${u.username}, ${hash}, ${u.role})
          ON CONFLICT (username) DO NOTHING
        `;
      }
    }

    return json(res, 200, { ok: true, message: 'Schema created and seed data inserted.' });
  } catch (err) {
